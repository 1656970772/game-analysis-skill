...
当研究范围包含多个参考作品、多个平台页面、多个本地目录或需要独立审查边界时，使用这些提示词分派并行任务。每个子任务都要限制在合法来源内：默认不下载、解包、提取或复用游戏资源；只有在用户明确授权本机学习目标目录时，才允许只读全量本地导出、索引、缩略图、关键角色动画帧整理和 EXE 画廊制作。Unity 本地导出任务按 AssetRipper 阶段、UnityPy 阶段和下游分析阶段分派，AssetRipper 必须先完成，UnityPy 随后执行。依据：用户任务授权本地学习提取；`copyright-and-style-boundaries.md` 定义了风格研究边界。

## 角色选择规则

主 agent 先根据研究模式、学习目的、引擎检测结果和当前阻塞点选择角色，不固定派满全部角色：
...
- 未经用户明确授权，不下载、不解包、不提取、不反编译任何游戏资源；已授权本机学习时，只写入用户指定学习目录。
