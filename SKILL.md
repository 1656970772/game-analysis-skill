...
公开发布包不含 `tools/`。使用本地工具前，先读取 `references/tooling-manifest.json` 并检查 `local_path` 是否存在。

- Python 依赖通过 `scripts/setup_python_env.ps1` 恢复。
- 其他第三方工具在用户允许联网、许可证允许、来源可信时，从 manifest 或官方来源获取，解压到 `tools/<tool-name>_<version>/`。
- 下载后记录 URL、版本、许可证、哈希、用途和版本输出；不可用时写入临时目录并继续只读流程。
...
当当前环境支持子 agent 工具，且任务可拆成互不重叠的阶段或产物时，主 agent 必须先建立任务图，再动态分派子 agent；不可用或任务强耦合时，主 agent 按同一任务图串行执行。参考 `superpowers:subagent-driven-development` 的做法：每个独立任务使用新子 agent，完成后先做范围/规格审查，再做质量/边界审查。

### 调度判定

- 适合分派：多引擎/多目录盘点、Unreal 分片导出、Cocos ASTC 批量解码、IL2CPP/native 与美术索引可并行、报告草稿与边界审查可并行。
- 暂不分派：授权路径未确认、study-root 未创建、主分支未检测、同一工具会写同一目录、下一步依赖当前命令结果、需要人工 GUI 操作且无法隔离。
...
4. 根据研究模式执行对应范围：`art` 只执行资源导出和分类，`code` 只执行代码提取，`full` 全部执行；能并行且写入范围互不重叠的任务才分派子 agent。
...
2. 查找可信工具：优先官方/开源/长期维护工具。
3. 下载到 Skill 的 `tools/`，记录来源和许可。
4. 小样本验证后再全量执行。
5. 若无法继续，写入阻塞项和下一步。
